...
- **The iPhone app no longer gets stuck zoomed-in after signing in to Google.** Returning from the Google sign-in screen could leave the whole app zoomed in with pinch-to-zoom unable to bring it back (only force-quitting fixed it). beanies now resets the zoom when it comes back to the foreground. Pinch-to-zoom still works everywhere else.
