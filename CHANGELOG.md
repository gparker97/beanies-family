...
- **Public holidays now show up in the daily briefing.** The Family Nook gets two new holiday surfaces wired off the country-aware holiday dataset that already powers the planner. On the day itself, a quiet Sky Silk banner sits above the orange briefing box with a warm greeting — "Merry Christmas, beans", "Happy Lunar New Year, beans", "Eid Mubarak, beans" and similar for a global allowlist of ~8 widely-recognised holidays, falling back to "Today is {holidayName}" for everything else. The matching emoji (🎄 🌸 🪔 🌙 🦃 🌷 💐 👔 🎆) sits beside it, and a small caption underneath reminds you that school and work may be off. The day before, the briefing list inside the orange box surfaces a top-pinned italic note — "💡 Tomorrow is Christmas Day — school & work may be off" — sized to wrap cleanly on mobile, with no checkbox or chevron (it's informational, not actionable). Both surfaces respect the existing Settings → Planner "show public holidays" toggle and disappear entirely when no country is set. Country-specific holidays (Vesak Day in Singapore, Bastille Day in France, etc.) still surface via the default "Today is X" line — no rewrite of the dataset needed.
- **The daily briefing box now has a name.** The orange critical-items card on the Family Nook used to be just an icon, a cycling beanie quote, and a count — leaving first-time users to infer what they were looking at. It now carries a "Your Daily Briefing" label in Outfit semibold uppercase, sitting inline before the activities/tasks summary so it doesn't steal any vertical height on mobile. Helps new users build a mental model of the box; helps existing users find their morning glance-spot faster.
