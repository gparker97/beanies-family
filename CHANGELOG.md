...
- **Photos now appear in an activity even if you close the drawer while it's still uploading.** Attaching a photo to an activity, closing the drawer mid-upload (before the "counting beans" spinner finished), and reopening it would show no photo and no spinner - the photo was actually saved in the doc, but the drawer's local view never picked it up until a full page refresh. Root cause: BaseSidePanel's drawer dialog uses `v-if="open"`, so closing the drawer unmounts the slot content (PhotoAttachments + its composable). The async upload kept running and wrote the photo straight into the doc, but the parent form's local `photoIds` ref only re-synced when the entity itself changed, not when its photoIds did. Now the binding watches the source `photoIds` content too, with the deep-watch suppressed during in-flight optimistic operations (so user-initiated adds/removes don't get clobbered by the same mechanism). The photo lands on reopen instead of needing a refresh.
