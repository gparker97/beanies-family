...
- **Clearer family-member invites.** The invite wizard now sets expectations up front ("next, you'll get a link + QR to send them") and the link screen makes it explicit to send _that_ link — and that the automatic Google Drive "file shared" email can be ignored. On the join screen, opening a valid invite now greets you ("You've been invited to join {family}") instead of the stale "you need a magic joining link", the Google Drive step is reframed as the clear final step ("Open your family file"), and it names the exact `.beanpod` file to pick.
