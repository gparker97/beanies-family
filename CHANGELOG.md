...
- **The confetti is a party popper now.** It fires from a corner of the drawer, arcs across the full width, and the pieces fall under gravity and settle along the bottom. The quiet scatter fades in across the card afterwards, once the floor has settled. Previously the pieces stopped in mid-air partway down, which is why it never looked quite right; nothing stops halfway down.
...
- **On the beanie wall, the confetti uses the whole drawer.** It was confined to the thin strip holding the event, so the shower stopped a couple of centimetres down and the rest of a very large panel sat empty.
