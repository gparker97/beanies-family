...
- The overview's progress counts every card in the deck, including the ones still to sort.
...
- **Scanning a code with the in-app camera now works from a normal distance.** Photos of a
  ...
  closer together than black and white. It now reads the colour the code is actually in, and
  falls back through several other passes before giving up.
...
- **You can now bring your existing Google Calendar events into beanies.** Connecting a calendar used to send your beanies activities out to Google but never brought anything in, so a family with a year of swimming lessons already in Google had to retype them, and typing them by hand left two of everything. There is now a one-time import: pick which calendars to read, see everything beanies found in the next 12 months, untick anything you do not want, and import the rest. Nothing is imported until you pick it.
- **An imported event you created is kept in sync rather than copied.** Editing it in beanies afterwards updates the original event in Google, so your calendar still holds exactly one of it. An event someone else created is copied instead, because beanies is not allowed to change it, and it stays labelled “copy only” in your planner so you are never left wondering why an edit did not reach Google.
- **A weekly swimming lesson arrives as one repeating activity**, not thirty separate ones, even when the lesson started two years ago. A few repeat patterns cannot be matched exactly, and those are imported once, on their next date, rather than being guessed at.
...
- **Birthdays keep to the people you are looking at.** Narrow the planner or the wall to one bean and the others' birthdays step aside with their events, instead of staying on screen alone. Pets' birthdays always show, because pets are not part of that filter.
- **A busy day still shows your own plans.** A month square has room for two all-day entries, and a day carrying both a birthday and a public holiday was using both, pushing everything the family had planned behind a "+2". Birthdays and holidays now take at most one of the two when there is something else to show.
...
- **The same event on two of your calendars is now one row.** An invitation appears on the organiser's calendar and on every guest's with the same identity, so scanning two calendars offered the school concert twice and would have imported two of it.
- **Re-running the import now shows what is new.** The list stopped at the first 200 events including ones already imported, so a family who had imported once was shown 200 greyed-out rows and told there was nothing new.
...
- **A pricing page.** `/pricing` sets out the whole model: 90 days of the entire app for every new family, then beanies basic at $30 a year or beanies + magic beans at $84.99 a year with the AI reader included. No free plan and no ads at any price, with self-hosting free forever. Every "free while we're in beta" line across the site now links to it, and the switching pages, help FAQ and terms no longer describe a free tier that isn't happening.
...
- **Celebration confetti is now confetti.** The flecks were bean-shaped, but at nine pixels across there is no room for the notch that makes a bean a bean, so they read as neither bean nor confetti. They are now a mix of rectangles, strips, curls and discs, still in the pod's four colours and still in pod order. Big celebrations use the same pieces.
- **Confetti no longer competes with the words on a card.** It sits back further, so a birthday still celebrates without making its own card harder to read. Drawers, which are mostly space, keep the fuller scatter.
- **Celebrating cards get the shower too**, then the pieces breathe gently instead of sitting still. On the beanie wall, cards settle and stay settled: a kitchen screen that never sleeps should not have something moving on it all evening.
- **The beanie wall now edits one list item at a time.** Tapping a second item while one was open left both inputs live, and tapping away from an item you had not changed left its box open indefinitely. Moving to another item saves the one you were on, exactly as the lists drawer already does.
- **The confetti in a celebration's details pane falls again, and it now looks like falling.** Opening a birthday was meant to rain beans in from above and settle them. Two things stopped it. The animation is spent once per activity per session so it does not twitch every time a card scrolls past, and the chip you tap to open the drawer had already spent it, so the panel opened frozen. Underneath that, the fall itself was under a centimetre, which read as beans drifting rather than confetti falling. A drawer now showers every time you open it.
- **The confetti is a party popper now.** It fires from both bottom corners of the drawer, arcs across the full width, and the pieces fall under gravity and settle along the bottom. The quiet scatter fades in across the card afterwards, once the floor has settled. Previously the pieces stopped in mid-air partway down, which is why it never looked quite right; nothing stops halfway down.
- **Dates on a travel plan's timeline are easier to read.** The date sat on a solid teal block in near-black text; it is now a quiet chip with the day number leading, matching the times listed beneath it.
- **A date now sits at the top of the bookings it covers.** With three things happening on one day it was centred against the middle one, so reading downwards you met the second booking before its date.
- **Both drawers use their whole space for the celebration.** On the beanie wall the confetti was confined to the strip holding the event, and in the app it stopped where the details ended, so a short activity left most of a tall panel empty. Both now fill the drawer, and behave identically.
...
- **Joining a family with a link now works when you supply the family file yourself.** If beanies could not fetch your family's file and asked you to pick it from your device, it then asked for a password instead of using your invite. Families created in the last few months have no password at all, so there was no way to finish joining. Your invite is what opens the file, and now it does.
- **The last screen in the join flow no longer offers to create a password.** Setting up your PIN after accepting an invite had a button reading "Create Password & Sign In". It creates a PIN, and now says so.
...
- **A reconnect from the Restore screen no longer says your data was saved**, because nothing was saved and the restore still needs starting again. It also stops leaving a developer error message on screen as the only thing you are told.
...
- **beanies now asks you to reconnect Google far less often.** Reconnecting on one device used to cut the connection on all your others, and on your calendar too, because Google's own mechanism works on the whole account at once rather than one device at a time. Each reconnect then pushed the next device into the same screen, and round it went. A dropped connection or a hiccup at Google's end could also start it, which it no longer can: only an actual refusal counts now.
- **You can restore your family file from Google Drive in the app.** The app offered only "Browse", the file picker on the phone itself, so a family whose file lives in Drive and who only ever use the app could not reach it at all. Both places are offered now, the same as on the web.
...
- **Tidying up your family file no longer refuses when everything is already saved.** It reported that changes had not reached the cloud yet, with the saved dot showing green, and then worked a minute later with nothing changed. beanies was marking the file as freshly changed at the exact moment it finished saving, and then reading that as unsaved work. It also now says plainly when it simply could not check, instead of claiming your work is unsaved, and offers Check Again either way.
- **Removing a family from one device no longer changes who owns it.** "Delete Local Family Data" promises the original file is not affected, but it also removed the family's entry from the beanies directory, and the next person whose device wrote to it was recorded as the owner. It only removes what is on that device now.
- **An invite sent from the iPhone app now opens.** The link carried an address that only existed inside that app, so nothing happened when the person you sent it to tapped it. Typing the code by hand always worked.
...
- **A change that beanies could not read back is no longer thrown away when a second tab is open.** In the narrow case where beanies could not tidy up its own stored copy because another tab was holding it, it went ahead and replaced everything from the family file, discarding what it had not read yet.
- **beanies no longer reports a problem to the team when you are simply offline.** Work waiting to be saved was being treated as work that had failed to save.
...
- **When the job is done, beanies says whether anything is left to do:** either that your other devices will pick it up on their own, or who still needs to update beanies. Never a bare "done".
- **A device that runs out of memory opening your family file now says how to fix it**, and where: the family file can be compacted from a device that still opens it, and the struggling one will then open it normally. It deliberately offers no button, because compacting needs more memory than opening, so the device that just failed is the one device that cannot do it.
...
- **Tapping a day in the week view opens that day.** Tapping one of the smaller days listed below the calendar brings it into the calendar instead — a day you can see gives you depth, a day you cannot gives you the day. Coming back from a day returns you to the week you left it from.
...
- **The wall uses the whole screen.** On a large display the calendar drew its hours at tablet size and left half the glass empty. The hour now grows with the screen — up to double — and the wall draws a full day of hours from top to bottom rather than only the stretch your events happen to cover. A big wall shows 6am to midnight, ruled hour by hour; a small one still shows your waking day.
- **Quiet stretches are only folded away when the day genuinely will not fit.** With room to spare the wall now draws the empty hours honestly, so the grid reads as a real day rather than a set of clumps.
- **The week's arrows sit either side of the dates they move**, instead of in the header, and there is a way back from a day to the week you came from.
...
- **Long events use a big screen properly.** Cards had a fixed maximum height chosen for a tablet, which cut a school day or a full shift short on a large display. That maximum now grows with the space available, up to a sensible limit. An hour is still exactly as tall on a quiet day as on a busy one, so the wall reads the same from across the room, and nothing changes at all on a smaller screen.
- **The week view now puts its cards down the side**, like the day and bean-lane views already did, which gives the calendar its full height back. The cards take their width first and the week shows as many days as still read properly beside them — three or four on a tablet, more on a big wall — with the rest listed underneath. Seven days crushed into a strip was worse than showing four you can actually read.
...
- **On a 1024x768 tablet, the bean-lane cards no longer paint over the last two hours of the day.** The calendar was running past the bottom of the space it was given, and on a short screen the cards below it were drawn on top. The cards now step down to a single line there, which gives the day back its height.
- **A folded "quiet until" band is never taller than the gap it replaces.** On a squeezed screen announcing a half-hour lull cost four times more room than drawing it, so the thing meant to relieve a crowded day was crowding it further — that is why a 1024-wide week showed 13-pixel hours with the event names sliced through.
...
- **Restoring a copy of your family file saved before a reorganisation now works.** It was doing the opposite: keeping what was on the device and writing that over the file you picked, so the restore appeared to do nothing and the older copy was gone. Restoring an ordinary backup is unchanged — it still merges what you picked with anything on the device that had not been saved yet.
...
- **Setting a PIN from the wall's setup card no longer does nothing.** On a device where a family file had been opened without signing in as a member, the Save button was silently inert: no message, no explanation, and closing the box returned to the same prompt. The card and the PIN form had disagreed about who you were. They now agree, and if beanies does not know who you are on that device it says so and points you at signing in first.
- **"Sign out and clear data" now always clears.** A safety check meant to protect unsaved work was also overriding the consent screen, so a device with unsaved changes kept its local copy after the user had explicitly asked for it to be deleted. That also removed the one way out of a device that could not open its family file.
- **A change made just after opening beanies could stop the file saving.** On a device whose local copy was slightly behind the copy in the cloud, an edit made before the background sync finished could clash with an edit that device had already saved, and beanies would then refuse to combine the two. The cause was a change not yet released, which gave each device a fixed identity; that has been turned off until it can be made safe. Nothing was lost when it happened, and saving recovers on its own once the app is reopened.
- **Sync stops less often, and says something truer when it does.** Several failures that fix themselves were being treated as permanent: a family file caught part-way through saving on another device, a file written by a newer version of beanies, and a slow moment on an older device. Any of those could stop beanies syncing for the rest of the session and show a message saying the data may be damaged. They no longer stop syncing, and the message now says what actually happened and that trying again usually works. beanies still refuses to save over a file it could not read, which was the point of the original change.
- **A file that could not be read no longer looks like a lost password.** Some failures were reaching the screen as "your password may have changed", or as an untranslated technical line, instead of the plain-English explanation written for them.
- Internal fixes to the family-file safety checks found by a review of the previous day's work. None had shipped to anyone.
- The Care & Safety and Emergency Contacts page headers are readable after dark. Both sat on a daytime pastel band that had no dark partner, so the white writing landed on a pale slab.
- **Two open tabs can no longer write over each other's changes.** A change not yet released made every tab of one browser write under the same identity, which the sync engine refuses to combine, and the save that followed would have replaced the family file with one tab's copy. Only one tab per family now holds that identity; any other tab works exactly as before. If a family file ever cannot be combined with the changes on a device, saving now stops and says so rather than writing over it.
...
- **The recipe form is grouped into four parts.** It had grown to eleven fields in one long scroll, so it now reads as _the dish_, _how to make it_, _finding it again_, and _your own touch_. The break between servings and ingredients falls where the form changes character, and the source link now sits with the dish it came from rather than adrift in the middle.
- **Pasting a list of tags now adds them one by one.** Pasting `quick, easy, vegan` used to make a single tag with commas in it.
- **Recipe tags now complete themselves as you type.** The list of tags you have used before was easy to miss and easy to out-type, which is how you end up with "family favourite" and "family favourites" as two separate tags forever. Start typing and matching tags appear; arrow keys and enter pick one.
...
- **Owners can now slim down a large family file (behind a developer flag while it is tested).** Years of edits make a family file slow and memory-hungry to open, which is why it can fail on an older tablet. This rewrites the file without its edit history: the same data, a fraction of the size. beanies saves a backup to your device first and refuses to continue unless that file actually landed, checks the rebuilt file matches the original before replacing anything, and other devices pick up the slimmer file the next time they are online. Off in production for now.
...
- **The Chinese translation had drifted, in three ways, and all three are fixed.** About a hundred phrases had slipped into traditional characters in a bundle that is meant to be simplified, so a mainland reader met a foreign script mid-sentence — in the validation messages, half the day names and the family-invite text. Nine phrases had lost the piece that gets filled in, so "Transfer → {account}" showed as a bare "transfer account" with the account name gone, and "at least {min} characters" showed the word "missing". And a handful had been translated into something the English never said: "Nice!" had become "you've sent your score to Facebook", "Enter your token" asked for a Yelp code, and "months" had become "million Hong Kong dollars". Also corrected: "Forecast" was the weather forecast, "Upcoming" was upcoming sports fixtures, "Next" on a wizard button said "next page", and "Leftovers" said "leftover rice". A check now runs on every build so none of these can come back.
...
- **A pod that is too big for a device now says so, instead of claiming your file is damaged.** Opening a large family file on a tablet without enough memory showed a "damaged file" error and then quietly deleted the local copy, which could not help and threw away the one copy that might have opened. beanies now tells you plainly that it is a memory limit on that device, that your file is fine and nothing has been deleted, and it leaves your data alone. The recovery screen no longer offers to clear your data in that situation, because clearing is the one thing that would lose it. Every way into the app says the same thing now, including signing in with a PIN, unlocking with your face or fingerprint, opening on a trusted device, redeeming a recovery kit, joining by invite and the ordinary start-up, so none of them can mistake it for a wrong password and ask you to type it again. If it happens while beanies is already open and showing your data, it says so plainly and leaves your data on screen rather than covering the app, and it stops trying to save over a copy it could not read. It also stops re-checking the file every ten seconds once it knows the answer, will not save over a copy of your family file it could not read (so another device's changes cannot be lost that way), and will not delete your local copy when you sign out while that is the only copy it can reach. Tapping Refresh, granting permission again or re-picking your file all give it a fresh attempt, so a one-off memory blip does not lock sync off for the rest of the session. And when the real problem is that your saved key is out of date, it now says so and points you at your password instead of telling you your data may be damaged. This does not yet make a large pod open on a small tablet: that needs the storage change tracked separately.

- **Saving and loading your family file uses noticeably less memory.** The file was parsed twice on every load, decoded in a way that held three copies of it at once, and kept a multi-megabyte copy in memory for the whole session and wrote it into local storage on every change. All of that is gone.
...
- Recipes captured from a web link now pick up the dish photo far more often. beanies used to look at only two places on a page and then throw away most of what it found — in particular any photo served from a CDN, which is how nearly every recipe site works. It now works through a list of the page's own images in order, tries each until one arrives, and accepts modern image formats it previously refused. Recipes captured from a video get the video's thumbnail.
- A recipe with no photo now shows one of several on-brand illustrations, chosen so the same recipe always looks the same while a cookbook page looks varied rather than showing one drawing over and over.
- When no photo can be found, beanies now records why. Previously the commonest outcome — a page offering nothing usable — was completely invisible, which is why this could quietly under-deliver until someone mentioned it.
- An App Store release no longer wastes a full signed build when a previous one is still waiting for review. Apple will not let a new build replace a version that is already in review, which is why the 0.16 submission uploaded fine and then failed at the last step. The release now checks first, says so in seconds, and can cancel and replace the pending submission when you ask it to.
...
- **Share text straight into beanies.** Select the details in an email or a class-group message, tap Share, and pick beanies — no attachment needed. beanies reads what you sent and works out whether it is an activity, a trip or a recipe, exactly as it does for a photo. beanies now appears in the iOS share sheet when text is selected. If you send more than beanies reads at once it says so; if you send a lot in a short time it tells you when it can read again.

- **Sharing a whole message now reads the message.** A school email whose details are in the body — but which also carries the school's website, a payment link and a signature — used to send beanies off to fetch one of those links instead. It now reads what you actually shared. Sharing a link with a few words around it still opens the link, exactly as before, and beanies can also read a good deal more text at once.

- **One magic-beans button.** The quick-add card's three separate buttons (invite, travel booking, recipe) are now a single one. Give beanies a photo, a file, a link, or some text you pasted, and it works out what it is — so you can no longer get a worse result by picking the wrong reader first.
...
- **The meal planner's week reads as a grid again.** Breakfast, lunch, dinner and snack each tint their whole row rather than a tiny label in the margin, days are separated by a column line, and the slot names are big enough to read. On a meal card the cook is now just their face, as on the calendar, which gives the meal name back the width it was sharing.
- **Eat out, leftovers and skip look like meals now.** They were flat coloured chips with no icon; they wear the same card as a cooked meal, with the icon they already had in the list beside them. A skipped meal still fades back, since it means nothing is planned.
- **A meal with no cook no longer says "Anyone"** — it just says nothing, which is what an empty field looks like.
- **Clearing a day is easier to hit.** The button sat on top of the day's name; it is now at the top of the column in its own space, and it shows up on a tablet rather than only on hover.
- **You can add more than one thing to a meal.** Breakfast, lunch and dinner only ever let you add one, with no way to add a second once something was there — most obviously on a phone.
- **A meal only says something once it has been cooked**, with a green tick on its picture, instead of carrying an orange dot the rest of the time to say it has not been.
- **The printed meal plan matches.** Rows are tinted the same way, the cook is a chip (the key at the foot still says who is who), and an empty slot is a line to write on rather than a dot.
- **Each bean's lane on the day view is now tinted their colour, all the way down.** It used to be a band of colour at the top that faded to white, so by mid-morning every lane looked the same. Cards inside a lane are now plain white with their owner's colour on the edge, so they stand out against it rather than blending in.
...
- Version 0.14 released to the web app and submitted to both app stores, carrying the PIN and Recovery Kit changes plus the continuous month calendar.
...
- **Sharing a photo or PDF on iPhone now opens beanies directly.** beanies registers as a handler for images and PDFs, so iOS hands the file straight to the app the way it does for other apps that open when you share to them. Links open the app too where iOS permits it, and fall back to a tappable notification where it does not.
- **Sharing on iPhone now nudges you straight back into beanies.** Tapping "Process in beanies" sends a notification you can tap to open the app and finish the import, which is the closest iOS allows to opening the app for you. If notifications are off it simply says to open beanies instead, rather than promising a nudge that never arrives.
- **Sharing to beanies on iPhone now confirms it worked.** iOS does not allow a share extension to open its app - that is a platform rule, not something we can work around - so the share sheet now says the item was captured and that opening beanies will finish it, instead of closing silently and looking like nothing happened. Links sent as plain text (which is how YouTube shares them) are accepted now too.
- **iPhone builds now ship with the permissions they were meant to have.** A signing shortcut in the release pipeline had been quietly stripping them since early August, which is why sharing to beanies did nothing on iPhone. The same gap had been silently breaking links that should open the app directly instead of the browser.
- **Sharing to beanies on iPhone works at all.** The share sheet accepted the item and then nothing happened - not even after opening beanies, because the item was never handed over. The app's own iPhone plugins were built but never connected to the app, so the app could not tell the difference between "this iPhone cannot do it" and "nobody plugged it in". The same gap is why Face ID unlock still had no effect on iPhone after the previous fix.
