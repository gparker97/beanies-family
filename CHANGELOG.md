...
- **A pricing page, built but not yet live.** `/pricing` sets out what stays free, what the AI helper will cost after beta, the 90-day trial every new family gets at v1.0, and the early-family offers. It is behind a `PRICING_LIVE` flag and on the `pricing-page` branch, so nothing changes on the site until the copy is signed off.
