...
- **Text casing**: All non-sentence text (buttons, labels, nav items, chips, page headers) must be lowercase in `uiStrings.ts`. Full sentences keep sentence case. CSS `uppercase` handles visual uppercasing for section headers and form labels. Rule of thumb: if it has no period, it's lowercase. See `.claude/skills/beanies-theme.md` § Text Casing Standard.
