...
> **Last updated:** 2026-04-26 (overnight — sign-out hang fix shipped (`3a38f2c`); two long-form plans + GitHub issues filed (`01a07d1`) for the joiner onboarding work after the iPhone Safari testing session uncovered that Picker is fundamentally fragile on iOS Safari due to ITP. Active hardening track filed as #185, long-term Drive UI Marketplace track filed as #186.)
>
...
> - **Original pod owner lost file access** (reported 2026-04-25 evening) — greg's wife or whoever created the pod can't read the `.beanpod` from Settings → reconnect (gets the same "file not found" error joiners get). **NOT addressed by anything shipped.** Hypotheses (in likelihood order): (1) file moved/deleted from Drive — easiest first check is to open drive.google.com and search `*.beanpod`; (2) OAuth grant revoked — check myaccount.google.com → Security → Third-party apps; (3) wrong Google account selected on the device. The "file not found" on a fresh OAuth attempt suggests authenticated but missing scope, which means the file is genuinely inaccessible at that fileId. Worth a single diagnostic session to determine which.
> - **Notion Launch HQ update** — still skipped multiple sessions (MCP not loaded in any of them). Manual touch-up should record: onboarding hotfix series (`a8f1a24`, `7e4c70b`), sign-out hardening (`3a38f2c`), and the planned hardening tracks (issues #185, #186). Block ID `32c247d9-a99f-8188-b169-eacfdc48c057`.
