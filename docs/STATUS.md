...
> **Last updated:** 2026-04-19 (The Pod P2)
> **Updated by:** Claude (session covered: The Pod — Phase 2 Bean Detail. Three sub-commits (P2-A through P2-C): shared pod components (StickyNote, OverviewModule, StatStrip, AddTile, EmptyState), `font-caveat` Tailwind theme var, content stores wired into app load, BeanDetailPage shell with BeanHero + BeanTabs + six-module Overview tab, Favorites/Sayings/Notes tabs with per-type form modals (categories + Caveat-body saying textarea + notes title/body), BeanCard click flipped to navigate-to-detail with edit-pencil affordance for the admin drawer. Plus three robustness passes on avatar photos: non-reactive `driveFileId` computed promoted to a ref, function-ref pattern on `useFilePicker`, authorized `alt=media` blob downloads replacing `thumbnailLink` for avatar URLs, photo spinner + orphan-upload cleanup, folder-share migration persisted in sessionStorage and filtered against unshareable emails, and `data/<familyId>/photos/` Drive subfolder layout. 1062 unit tests green.)
>
