...
> **Last updated:** 2026-04-25 (evening — onboarding hotfix: invitee join flow unbroken on iOS Safari + shared Drive folders. Two real-user-blocking bugs found by greg's wife while testing a fresh invite. Vue prod deploy `a8f1a24`.)
...
>
> - **Real-device validation of the onboarding hotfix on iPhone Safari.** Shipped 2026-04-25 evening on commit `a8f1a24`. Send yourself a fresh QR invite from a logged-in browser, scan it on iPhone, expect to see the "Pick from Drive" CTA (NOT the "popups are blocked" error). Tap → full-page redirect to Google → land back on `/join` with `fileId=...` in the URL → file should auto-load by fileId without forcing the user through the Picker. If the auto-load fails, the Picker should open with `.beanpod` files visible (My Drive + Shared with me views). Pick the shared `.beanpod` → it should load (no "doesn't have a pod inside" error). If anything regresses, the relevant code is in `src/components/login/JoinPodView.vue` (the `attemptFileLoad`, `handlePickFromDrive`, and `handlePickFromDriveWithToken` functions) and the helpers in `src/services/google/googleAuth.ts` (`shouldUseRedirectAuth`, `tryGetSilentToken`).
> - **Notion Launch HQ update for the onboarding hotfix.** Skipped this session — the Notion MCP wasn't loaded, so the launch-status update couldn't be made automatically. Worth a manual touch-up in the Launch HQ to note that the iPhone-Safari onboarding bug + the shared-folder false-negative are now fixed in prod (commit `a8f1a24`, deploy run `24930704417`). These were both real-user-blocking bugs surfaced during a live invite test, not theoretical issues. Block ID for Launch HQ from the end-session skill: `32c247d9-a99f-8188-b169-eacfdc48c057`.
