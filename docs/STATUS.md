...
> - **Substack rename — Greg to action.** Change `gpbeanies.substack.com` → `everybeancounts.substack.com` in Substack admin (one-shot domain change). Also update publication name + bio to match the brand voice. Not a repo change; reminder only.
