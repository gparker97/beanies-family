...
**🔴 E2E regression fixes (user-flagged for next session):**

- The E2E suite needs a pass. User noted "remind me to fix e2e errors when i come back" at session close on 2026-04-21. Context:
  - `docs/E2E_HEALTH.md` already tracks a logged WebKit onboarding flake from 2026-04-20.
  - Current suite at 33 tests (over ADR-007's 25-test cap) — candidates for consolidation documented as `cross-entity.spec.ts:81` + `:333`.
  - Any failures introduced by this session's feature work (medication administration log — new view drawer, confirm dialog, card restructure with two tap targets) should be triaged against the existing specs. None of this session's unit tests surface E2E failures directly, but card structural changes + modal swap to drawer are plausible E2E destabilizers.
  - **First step next session**: run `npm run test:e2e` locally, log outcomes in `docs/E2E_HEALTH.md` per the ADR-007 (a/b/c) classification.
