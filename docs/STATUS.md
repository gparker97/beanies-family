...
> **Last updated:** 2026-04-25 (evening, 2nd round — onboarding hotfix follow-up: auto-recover from `drive.file`-scoped 404 by silently opening the Picker, plus iOS redirect-loop fix. Vue prod deploys `a8f1a24` then `7e4c70b`.)
>
...
>
> - **Re-test onboarding on iPhone Safari end-to-end** (2nd round) — shipped on commit `7e4c70b`. Greg already confirmed the popup error is gone after `a8f1a24`; this round adds the Picker-auto-recovery on 404. Expected flow on a fresh QR scan: tap "Pick from Drive" → no popup error → redirect to Google (or skip if cached) → return to /join → spinner → if auto-load 404s, Picker auto-opens → `.beanpod` visible under "Shared with me" → pick → load succeeds → continue. **Watch for**: any regression where the Picker opens empty (means iOS Google account ≠ inviter's share-target email — UX could surface this but isn't in scope for this hotfix); any case where the redirect-loop kicks in (means the `tryGetSilentToken` fix in `handlePickFromDrive` didn't catch).
> - **Long-term fix: anyone-with-link on the `.beanpod` to skip the Picker step entirely** — the cleanest architectural fix is to mirror the photo approach. Inviter-side `MeetTheBeansPage.vue:243-272` (`handleShareWithEmail`) and any other invite-generation paths should also call `setPublicLinkPermission(token, syncStore.driveFileId!)` on the `.beanpod`, alongside the existing email-based share. The function exists at `src/services/google/driveService.ts:296-302` and is already used for photos. **Why it's safe**: the `.beanpod` is encrypted with the family key + invite-key-wrapped — anyone-with-link readers see only ciphertext, no different from a photo CDN URL. **Migration needed for existing pods**: a one-time sweep on next app launch that calls `setPublicLinkPermission` on the user's own `.beanpod` if they own one. Pattern exists in `src/composables/useEnsurePhotosPublic.ts` (already does this for photos). **Decision needed**: greg should sign off on the privacy stance before shipping. Once landed, joiners' auto-load by fileId would Just Work — no Picker step at all. The Picker fallback in `JoinPodView` would still exist as belt-and-braces for old pods.
> - **iOS account-mismatch UX polish** — if a joiner is signed into Google on their iPhone with an account different from the email the inviter shared with, the Picker will come up empty (file not visible because it's shared with a different email). Today they'd see the Picker, find nothing, hit "Or load from your device" — which works but is confusing. A small UX win would be to surface the joiner's signed-in Google account email after auth and prompt: "you're signed in as `<email>` — make sure the inviter shared with this exact address". Reuse `getGoogleAccountEmail()` from `googleAuth.ts:537`. Low-priority but a real-user friction point worth noting.
> - **Notion Launch HQ update** — still skipped (MCP not loaded). Manual touch-up to record both onboarding hotfix rounds (commits `a8f1a24` + `7e4c70b`, deploys `24930704417` + `24931001526`). Block ID `32c247d9-a99f-8188-b169-eacfdc48c057`.
> - **Dead code cleanup from the onboarding hotfix (low-priority).** `pickBeanpodFolder` in `src/services/google/drivePicker.ts:55-134` and `findBeanpodInFolder` + `NoBeanpodInFolderError` in `src/services/google/driveService.ts:504-541` are now unused in production code (and so is the `join.pickerPrompt.noBeanpodInFolder` translation key in `uiStrings.ts`). Their unit tests in `src/services/google/__tests__/driveService.test.ts:401-440` still pass. Kept around in case the photo-access architecture needs them again, but if no use case materialises in the next month, safe to delete along with their tests + the translation key.
