...
- **4 pillar guides — residual cleanup before flipping `draft: false`:**
  ...
  - **`/frontend-design` review of the guides section.** Greg wants a design pass on how the pillar guides render (typography, spacing, TOC behavior, short-answer blocks, link styling) and a discussion about how the guides section integrates into the broader beanstalk (blog) section — navigation, hub-and-spoke linking, "further reading" template behavior, whether to promote the pillars above blog posts on the `/blog` landing, etc.
