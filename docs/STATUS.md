...
>
> - **Mobile nav redesign — implementation.** v2 mockup is at `docs/mockups/mobile-nav-bean-jar-v2.html`. Pattern law: cluster bloom = ADD (FAB only, with `+` seal on every bean); pure bean stack = NAVIGATE (bottom nav only, single column). Wire into `src/components/common/MobileBottomNav.vue`: 4 tabs (Nook / Planning / Money / Pod); Nook is single-tap-to-`/nook`; the other three toggle a vertical bean stack rising from the tapped tab. Active route bean ringed in Heritage Orange and placed at the bottom of the stack (closest to thumb). Floating eyebrow ribbon above the stack ("🌳 _planning_ · 3"). Cap at 6 visible items; if a category exceeds 6, the 6th slot becomes a tinted `＋N more` bean opening a popover. Hide Money tab when `canViewFinances` is off (collapse to 3 tabs). Same-tab-tap / scrim / Esc / scroll / back-gesture all close. Reduced-motion fallback = 250ms fade. Update `MOBILE_TAB_ITEMS` in `src/constants/navigation.ts` (currently 6 items, needs the 4-category structure with children — likely just reuse existing `TREEHOUSE_ITEMS` / `PIGGY_BANK_ITEMS` / pod children groupings). i18n keys for new tab labels (`mobile.planning`, `mobile.money` if not already present). **Open calls**: tab naming ("Planning" vs "Treehouse" — Greg to call); overflow popover style (right-anchored mini-popover vs bottom sheet — revisit only if a category actually crosses 6).
