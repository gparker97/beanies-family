...
**🟡 E2E triage (reduced scope — critical fix shipped):**

- `cross-entity.spec.ts:402` "Data integrity: add then remove linked payment" was fixed in `6112103` (hardcoded past date → runtime tomorrow). Remaining known-flaky specs documented in `docs/E2E_HEALTH.md`: `cross-entity.spec.ts:81` ("Custom institution persists and appears in asset lender dropdown" — pre-existing, consolidation candidate) + the WebKit onboarding flake from 2026-04-20.
- Full E2E run hasn't been executed against the goal-activity-log changes yet. Activity log + contribute flow + delete + remaining-amount rename haven't been exercised by E2E. Likely safe since the relevant tests target financial-data paths rather than goals, but worth a one-off run if anything looks off.
- Current suite: 33 tests (over ADR-007's 25-test cap). Consolidation still pending.

**🟢 Goal-activity-log follow-ups (from plan, explicitly deferred):**
...
**🟢 Astro deploy in flight at session close:**

- `fe268a2` greg.astro bio refresh triggered Astro deploy-web run `24722151908` which was still in-progress when the session wrapped. Unless it failed, no action needed; otherwise re-trigger via `gh workflow run deploy-web.yml --ref main`.
