...
**Decision: photos are uploaded with `anyone-with-link → reader` Drive permission.** Every photo upload in `photoStore` (regular attachments, avatars, replacements) immediately follows `createFile` with `setPublicLinkPermission`. Rendering uses direct Drive CDN URLs — `drive.google.com/thumbnail?id=...&sz=wN` for thumbnails, `drive.google.com/uc?export=view&id=...` for full-size — so no OAuth token is required to fetch bytes.
