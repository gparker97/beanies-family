...
#### `drive.file` scope implications — public-link access for photos

The app uses the `drive.file` OAuth scope (per-file authorization), not the broader `drive` scope. This scope is deliberately narrow — the app can only API-fetch files it created in that user's Drive, or files the user explicitly picked via Drive Picker / "Open with." Two consequences shaped the photo-rendering design:

1. **Folder sharing does NOT grant the joining member's app API access to sibling files.** Sharing the `beanies.family` folder with `user@example.com` makes the folder and its contents visible at drive.google.com, but the member's app still 404s on every file inside it that the app itself didn't create and the member didn't individually pick. Folder-level Drive permissions and OAuth-scope-level app permissions are orthogonal.

2. **Folder-picking via Drive Picker does NOT extend `drive.file` to descendants created by other users.** Picking the `beanies.family` folder grants the app permission to list the folder and create new files in it — but existing files other users put inside (the photos) remain outside the app's reach. Tested empirically during the first rollout of this ADR; the folder-pick-based "recovery" flow sent users in a loop where validation succeeded on `.beanpod` (already individually picked during join) but photos kept 404ing.

**Decision: photos are uploaded with `anyone-with-link → reader` Drive permission.** Every photo upload in `photoStore` (regular attachments, avatars, replacements) immediately follows `createFile` with `setPublicLinkPermission`. Rendering uses `https://lh3.googleusercontent.com/d/{driveFileId}=w{N}` — Google's image CDN — so no OAuth token is required to fetch bytes. (The `drive.google.com/thumbnail?id=...` and `drive.google.com/uc?...` URLs looked simpler but were session-sensitive and thumbnail-generation-dependent: fresh uploads returned errors before Drive had generated a thumbnail, and anonymous loads could get bounced to a sign-in page even for anyone-with-link files. `lh3.googleusercontent.com` doesn't have either issue.)

- **Migration**: `useEnsurePhotosPublic()` (wired from App.vue) runs once per session after `syncStore.driveFileId` resolves. It iterates every photo in the Automerge doc and calls `setPublicLinkPermission`. Idempotent; 403 responses (not this user's file) are skipped silently — that file's owner runs the sweep on their own device. Session-scoped guard (sessionStorage, keyed on driveFileId) prevents re-running every navigation.

- **Join flow**: unchanged by this decision, but `pickBeanpodFolder` + `findBeanpodInFolder` (introduced earlier) stay — folder-picking is still marginally cleaner UX than file-picking for the join path, and the `.beanpod` is still the single pick the joining user needs to make.

**Privacy analysis**: photo bytes are fetchable by anyone who has the direct Drive URL. driveFileIds are random 33-character strings (not guessable). The URLs live inside the Automerge doc, which is encrypted at rest with the family key (AES-GCM; key derived from passkey or password, never on any server). To view a photo a non-member would need both (a) the encrypted Automerge doc from Drive — gated by the owner's Drive-level permissions — and (b) the family key. Effective exposure: members only. This is the same trust boundary as every other piece of family data. The rejected alternative (broader `drive` scope) would have been strictly worse on privacy — the app would have read access to the user's entire Drive instead of just the files it creates.
