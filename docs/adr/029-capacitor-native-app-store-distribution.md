...
2. **OAuth runs in the system browser, not the WebView.** Google blocks embedded-WebView OAuth (`disallowed_useragent`). We route Google sign-in through the system browser via `@capacitor/browser`, returning to the app through a **verified https App Link** (`https://beanies.family/oauth/native`). The `redirect_uri` registered with Google must stay https — the OAuth client is a Web-application type, which rejects custom schemes (verified 2026-05-22) — and an App Link reuses the Web client + Lambda exchange while being hijack-resistant. The existing ADR-026 redirect flow is the foundation; the redirect _target_ becomes the system browser and the _return_ becomes a deep link. **This is the highest-risk integration and is validated first in the spike.**

   > **AMENDED 2026-08-06 — two corrections, both found the hard way on iOS TestFlight builds 7 and 8.**
   >
   > **(a) This decision said iOS uses `ASWebAuthenticationSession`. It does not.** `@capacitor/browser` uses **`SFSafariViewController`** on iOS (`Browser.swift:20`). The distinction is not cosmetic: `ASWebAuthenticationSession` has a native callback mechanism, `SFSafariViewController` does not. Believing the former is a large part of why the failure below was not predicted.
   >
   > **(b) "NOT a custom scheme" was right about Google and wrong about the return hop.** Apple fires Universal Links only on **user-initiated taps**. An OAuth _redirect_ to one inside `SFSafariViewController` never hands off — it just loads the page in the browser sheet. With the apex 301ing `/oauth/native` to `app.beanies.family`, that meant the whole PWA loaded _inside the sheet_, hit a route the Vue router did not have, rendered its 404, and bounced to `/welcome`. Every screen after that was running in the browser sheet, not the app.
   >
   > **The fix is a bridge, not a new `redirect_uri`.** Google still redirects to the https URL it has registered (so (a) above is unchanged); the page served there hops to `family.beanies.app://oauth/native`, which custom schemes _can_ do from a browser sheet, and which Capacitor delivers as the same `appUrlOpen` event. Android keeps using its verified App Link and registers the scheme only as a recovery path.
   >
   > **Security consequence:** `handleNativeAuthRedirect` is no longer reachable only via an OS-verified link — any installed app or web page can invoke the custom scheme with arbitrary params. The CSRF `state` nonce is therefore the **primary** authenticity control, not defense-in-depth. Do not remove it.
   >
   > `ASWebAuthenticationSession` remains the better long-term design, but its https callback is **iOS 17.4+** (we target 15.0) and needs a `webcredentials` association we retired with the passkey pivot. Full analysis: `docs/plans/2026-08-06-ios-oauth-custom-scheme-bridge.md`.
...
**IMPLEMENTED (2026-07-14, plan `docs/plans/2026-07-14-native-biometric-keystore-pivot.md`).** Shipped a custom first-party `BiometricKeystore` Capacitor plugin — Android `BiometricKeystorePlugin.java` (AES-256-GCM `AndroidKeyStore` key, `setUserAuthenticationRequired` + `setInvalidatedByBiometricEnrollment`, per-op `CryptoObject` binding via `BiometricPrompt`, StrongBox best-effort, API-30+/24–29 auth-param branch) and iOS `BiometricKeystorePlugin.swift` (biometric-gated Keychain item, `SecAccessControl(.biometryCurrentSet)` + `WhenUnlockedThisDeviceOnly`, `LAContext`). JS: `nativeBiometric.ts` owns the native path; `passkeyService.ts` delegates to it via an `isNative()` guard at each entry point (web WebAuthn-PRF + all orchestration/UI untouched); shared cool-off/label/`describeAuthError`/`tr` extracted to the leaf `biometricShared.ts` (no import cycle). The wrapped blob is device-local (never synced); records carry `mechanism: 'native-keystore'`, one per family per device (last-writer-wins).

> **AMENDED 2026-08-27 (tracker #76, plan `docs/plans/2026-08-27-biometric-unlocks-and-signs-in.md`): the keystore item is now PER MEMBER, not per family.** The account is `${familyId}:${memberId}`, so two members sharing a device can each enrol and each be signed in as themselves; enable no longer evicts a sibling's record. The original one-per-family rule assumed biometric only ever had to _decrypt_ the pod — true of the key, but not of the job: it also identifies who is signing in, and a family-keyed item cannot say which member. That assumption is what made biometric silently unavailable on the commonest path of all (pod already decrypted after a sign-out).
>
> Devices enrolled under the old scheme are **not migrated** — each record carries a `keystoreScheme` field saying which address holds its key (absent = the legacy bare-`familyId` address), and legacy enrolments keep working there indefinitely. A re-enrol moves a record to the per-member address; `nativeReclaimFamilyKeystore` deletes both forms on family delete.
>
> A read-repair migration was designed and rejected during review. It looked free — "`setKey` needs no authentication, so re-homing the key costs no extra prompt" — and that is true only on iOS. On Android `setKey` generates a fresh auth-bound key and calls `promptAndFinish`, i.e. a SECOND `BiometricPrompt` labelled _"Enable biometric unlock"_ immediately after the unlock the user had just satisfied; and because the legacy blob is only deleted after that write succeeds, dismissing it would bring the double prompt back on every launch, forever. Recording the address is both simpler and honest about the platform difference.
>
> Unchanged, and worth restating because the per-member key does NOT alter it: the OS prompt still authenticates the DEVICE, not a person. If two people are enrolled at OS level, either satisfies it. The mitigation is product, not cryptographic — an always-visible "not you?" escape shown _before_ the user lands in an account.
>
> `nativeBiometric.ts` is the only module permitted to construct a keystore account string (per-member or legacy). Other modules reclaim storage via `nativeReclaimFamilyKeystore(familyId)`, which deletes every per-member blob plus the legacy one — a second address-aware file is how a future migration silently orphans blobs. Retired: the `@capgo/capacitor-passkey` plugin + Swift PRF patch + `patch-package`/`postinstall`, the `main.ts` shim, the `capacitor.config.ts` `CapacitorPasskey` block, `capacitor-passkey.xml`, the `asset_statements` meta, the app-level `androidx.credentials*`/`androidx.webkit` deps, the iOS `webcredentials` entitlement + app-origin `webcredentials` AASA, and the `get_login_creds` assetlinks relation (OAuth App Links / `handle_all_urls` kept). New `native-biometric` telemetry surface (+ `key_backing` allowlist key, client + Lambda + pinned test). iOS Swift compiles at the first `mobile-ios-release`; Android builds in CI now.

> **AMENDED 2026-09-15 (tracker #82, plan `docs/plans/2026-09-15-ios-keychain-orphan-reconcile.md`): the KEYCHAIN, not IndexedDB, is the durable index of key material — and reclaim now works from the union of both.**
>
> iOS keychain items survive an app uninstall by design. The `passkeys` IndexedDB store that enumerated them does not: it lives in app storage, which iOS deletes. So after a delete-and-reinstall, per-member blobs existed on the device with no code path able to see, use or remove them — "clear all data", signing out and removing a member all missed them, which for a product whose promise is that the family's data is theirs is a promise the app could not keep.
>
> The resolution is that a separate durable registry was never needed. Every item shares one fixed service (`family.beanies.app.biometric`) and the account already encodes both ids, so a query by **service alone** (`kSecMatchLimitAll` + `kSecReturnAttributes`, and deliberately no `kSecReturnData`) recovers every `familyId`/`memberId` pair on the device. The plugin simply never exposed that: `baseQuery()` always pinned `kSecAttrAccount`. "Beyond any code path" was true of the plugin's API surface, not of the platform.
>
> **The fix is ADOPTION, not deletion.** At the login seam, orphaned blobs are written back into the registry; once a record exists, every deletion path the product already advertises reaches the blob. That cures the defect without deleting anything, and a legitimate reinstaller keeps their biometric unlock instead of being silently broken. Adoption only makes material visible — it grants no access the device's owner did not already have, since the blob is still biometry-gated and `ThisDeviceOnly`.
>
> Consequently `nativeReclaimFamilyKeystore` is **union-driven**: enumerated blobs for that family, plus registry-derived targets, plus the legacy address. A failed or unavailable enumeration contributes zero members, which _is_ the fallback — one code path, no branch to forget, and behaviour monotonically at least as good as before on every platform. It also keeps the converse hole closed: a purely keychain-driven reclaim would stop removing records whose blob the OS had already wiped, leaving dead records rendered as dead buttons on the chooser.
>
> **Clear-all-data sweeps the whole service** on both platforms via a new `deleteAllKeys` — one delete over class + service with no account, so it needs no authentication and cannot miss a gated item. That deliberately makes the product's strongest promise independent of the enumeration's prompt behaviour, which is the least certain assumption in the whole design. It carries a `deleteKey`-only fallback for the #74 class (a Swift method that exists but was never added to `pluginMethods`), because a build where the sweep rejects would otherwise delete the registry records and report a clean device while every blob survived. Android implements `deleteAllKeys` too: an iOS-only method would have rejected there while the records were still deleted, newly introducing this exact bug class on the platform that never had it.
>
> `listAccounts` stays **iOS-only**, and that asymmetry is safe where `deleteAllKeys`'s was not: Android's blobs live in `SharedPreferences`, which dies _together with_ the IndexedDB registry, so the two can never diverge and there is nothing to enumerate back.
>
> Adoption runs at exactly **two named seams** (`nativeResolveDeviceKeys` and `nativeUnlock`), single-flighted and bounded by **one** `raceTimeout` budget per session rather than per caller — `FamilyPickerView` mounts at launch on native and loops families sequentially, so a per-caller timer would have multiplied the budget by the family count before first paint.
>
> Accepted residue: a user who reinstalls and never signs in again keeps their own old blobs, because sweeping before sign-in would delete a legitimate reinstaller's key. The material is biometry-gated, device-local and `ThisDeviceOnly` (so it cannot be restored onto other hardware), and clear-all-data gives an explicit route to remove it.
>
> Unchanged: `nativeBiometric.ts` remains the ONLY module permitted to construct a keystore account string — and now also the only one permitted to **parse** one. Legacy blobs are still never re-homed (the reason is recorded in the 2026-08-27 amendment above and has not changed); they are enumerated, counted and reclaimed, never adopted, because a legacy account encodes no member and `PasskeyRegistration.memberId` is required.
