...
- **Data-minimization.** Send **only the single document** the user pointed at — never the family dataset. Compress/down-scale client-side first.
  - **Update 2026-09-25 (#107, bank statements): one named exception.** The `statement` task also sends the family's **merchant memory**: merchant names from past transactions (normalised, at most 150, ordered by how often they occur) with the category id each was last filed under, plus the app's own category id list. **Nothing else**: no amounts, dates, account names or numbers, balances, or member names. Why: the statement reader's suggested category is only useful if it matches the categories this family already uses, and the early-adopter ask ("use previous entries from the same merchant to auto-fill the expense account") cannot be met from the statement alone. How it stays inside this ADR's spirit: it is (1) scoped to the one task (`ExtractionRequest.context`, ignored by every other task), (2) disclosed on a dedicated statement consent sheet that states it and the read count, and shown ONCE even to a family that skips the generic prompt (`aiStatementConsentAcknowledgedAt`), (3) rendered in the SYSTEM message outside the untrusted fence, each name stripped of fence markers, and (4) on the managed tier built into the sealed body client-side, so our proxy never sees it in the clear. Decided by greg in the #107 pre-plan; plan `docs/plans/2026-09-25-statement-import-review-and-reconcile.md`.
