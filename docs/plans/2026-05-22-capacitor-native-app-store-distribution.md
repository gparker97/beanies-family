...
3. **Greg-side prerequisite for OAuth:** the native redirect URI must be registered in the existing Google Cloud Console OAuth client. **The redirect is an https App Link, NOT a custom scheme** — verified during implementation (2026-05-22): the existing "Drive integration" client is a **Web application** type, and Google rejects custom schemes (`app://…`) on Web clients ("must use either http or https as the scheme") — those are only allowed on iOS/Android/Desktop client types. Creating a separate native client would abandon the reuse of the Web client + Lambda exchange, so instead we register the **App Link** `https://beanies.family/oauth/native` (https — accepted) and route it into the app via a hosted, verified `/.well-known/assetlinks.json`. This is strictly better than the originally-planned custom scheme: a verified App Link can only be claimed by our domain-verified signed app, so the custom-scheme interception/injection vector is closed at the transport layer; the A2 `state` check stays as cheap defense-in-depth (and covers the pre-verification / fallback-to-browser case). `assetlinks.json` is one static file on the domain we already control; `apple-app-site-association` does the same for iOS Universal Links (Phase B). **The custom scheme is dropped — not a fallback.**
...
2. **OAuth redirect mechanism:** an https **App Link** (`https://beanies.family/oauth/native`, verified via `assetlinks.json`), registered on the existing **Web** OAuth client (Caveat 3 — custom schemes are rejected by Web clients, so the custom scheme is dropped, not a fallback). iOS uses the same URL as a Universal Link (Phase B). A2's `state` validation is defense-in-depth on top of the verified-link transport.
...
- **Google Cloud Console:** add the App Link `https://beanies.family/oauth/native` (https) as an authorized redirect URI on the existing Web OAuth client. (Custom schemes are rejected by Web clients — do not attempt to add `family.beanies.app://oauth`.)
...
**A2. System-browser OAuth deep-link.** In `src/services/google/googleAuth.ts`, add a native-only `startNativeAuth(returnPath, loginHint?)` that mirrors `startRedirectAuth()` exactly — same `generateCodeVerifier`/`generateCodeChallenge`, same `buildAuthUrl(clientId, codeChallenge, 'consent', loginHint)` (the `prompt=consent` invariant is load-bearing, see ADR-028), same `sessionStorage` write of `{ codeVerifier, returnPath }` under `REDIRECT_AUTH_KEY` — but instead of `window.location.href = authUrl`, set `redirect_uri = https://beanies.family/oauth/native` (the verified App Link, not a custom scheme — see Caveat 3) and open via `Browser.open({ url: authUrl })`.

**Reuse the existing completion — do not write a new exchange.** Register one native-guarded `@capacitor/app` `appUrlOpen` listener (registered exactly once via a module-level idempotency guard mirroring `usePwaUpdater`'s `initialized` flag and the auth-wake listener's install guard — store the returned `PluginListenerHandle` for removal in a `__resetNativeAuthForTesting()` hook, so a future `App.vue` remount can't stack a second listener that double-consumes the OAuth code) that, on a `https://beanies.family/oauth/native?...` deep-link URL: parses `code`/`error`, writes `code` to the shared redirect-code sessionStorage key, calls `Browser.close()`, then invokes the **existing `completeRedirectAuth()`** — which already does PKCE-verifier reload, the Lambda `exchangeCodeForTokens`, the `drive.file`-scope check, refresh-token persistence, `scheduleAutoRefresh`, and `notifyTokenAcquired`. The only web↔native difference is the _transport_ (deep link vs. full-page redirect into `OAuthCallbackPage`); the completion is shared verbatim. **Hardening:** promote the bare `'beanies_redirect_auth_code'` literal to an exported const in `googleAuth.ts` (next to `REDIRECT_AUTH_KEY`) referenced by all three touch-points — `OAuthCallbackPage.vue`, the native `appUrlOpen` handler, and `completeRedirectAuth`'s reader — so the native path can't drift from the web path on a renamed key.

**Security — add a `state` param for the deep-link transport (defense-in-depth; the one place the native path must NOT mirror the web path).** The existing flow carries no `state` (verified: `buildAuthUrl` never sets it; neither `completeRedirectAuth` nor `OAuthCallbackPage` validate it) — acceptable on web (the code returns via a same-origin top-level navigation). The native redirect is a **verified App Link**, so a co-installed app cannot claim `https://beanies.family/oauth/native` (Android verifies domain ownership) — the custom-scheme interception/injection vector is closed at the transport layer. `state` is still added as cheap CSRF defense-in-depth and to cover the pre-verification window and the fallback-to-browser case. The native flow: (a) generates a CSPRNG `state` (reuse `generateCodeVerifier()`), persists it in the `REDIRECT_AUTH_KEY` object, and passes it via a new optional `state?` param on `buildAuthUrl` (web callers pass nothing, unaffected); (b) in `appUrlOpen`, **validates the echoed `state` against the persisted value before writing the `code`** — on mismatch: discard, `clearGoogleSessionState()`, `reportError({ surface: 'native-oauth-state-mismatch' })`, never exchange. Gate validation on `state` presence so existing web flows/tests are untouched.
...
**Correctness fix the implementer must not miss:** `getRedirectUri()` returns `${origin}/oauth/callback`, which is wrong for the native exchange — the token exchange must echo the same `redirect_uri` Google saw. Add a native branch to `getRedirectUri()` returning `https://beanies.family/oauth/native` so `startNativeAuth` and `completeRedirectAuth`'s exchange agree (single touch-point; do not thread a `redirectUri` param through callers). Without this the exchange fails with `redirect_uri_mismatch`.

Add an **autoVerify App Link intent-filter** for `https://beanies.family/oauth/native` to the Android `AndroidManifest.xml` (and the matching iOS associated-domain in Phase B) — no custom scheme. The deep link only routes into the app once `assetlinks.json` is hosted with the app's signing-key SHA-256 (debug + release fingerprints during dev); before that the redirect lands on the website, so `https://beanies.family/oauth/native` needs a minimal fallback page that hands off to the app / app store.
...
- `web/public/.well-known/assetlinks.json` — Android App Link verification (app signing-key SHA-256, debug + release); the iOS `apple-app-site-association` equivalent in Phase B. Hosted on `beanies.family`; static-only, no runtime/deploy-pipeline change.
- A minimal `https://beanies.family/oauth/native` fallback page (Astro) — for the pre-verification window and the app-not-installed case (hands off to the app / app store); does not affect the verified-app deep-link path.

**Modified:**

- `capacitor.config.ts` — App Link host config, plugin config, server settings (no custom scheme).
...
- `android/app/src/main/AndroidManifest.xml` — autoVerify App Link intent-filter for `https://beanies.family/oauth/native` (no custom scheme), permissions.
...
Implementation-time discoveries that refine the approved plan (logged here rather than re-running the four passes, since each simplifies or corrects a localized mechanism without new scope):

- **2026-05-22 — native OAuth redirect is an https App Link, not a custom scheme.** Registering `family.beanies.app://oauth` on the existing Google OAuth client failed: "Invalid Redirect: must use either http or https as the scheme." The client is a **Web application** type, which only accepts http/https redirect URIs (custom schemes are for iOS/Android/Desktop client types). A separate native client would abandon the Web-client + Lambda-exchange reuse, so the redirect is now the verified App Link `https://beanies.family/oauth/native` (https — accepted) throughout (Caveat 3, Assumption 2, Track 0, A2, Files Affected updated). This is a security improvement (a verified App Link can't be hijacked like a custom scheme) and a simplification (single transport, no dual-path). `state` validation is retained as defense-in-depth.
