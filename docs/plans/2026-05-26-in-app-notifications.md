...
1. **Header bell with unread badge.** A new squircle bell joins the existing header controls (desktop: among privacy/search/profile in the right-side squircle cluster of `AppHeader.vue`; mobile: in the mobile header section of the same component, per the design-system spec). It shows a **Heritage Orange** unread indicator (default: a calm dot; an unread-count variant is a one-token swap) whenever ≥1 notification is unread for the current member. The badge clears the instant the last notification is read.
...
- **`NotificationsBell.vue`** — header button + badge (dot default / count variant), calls `store.open()`/`store.close()`. Placed in `AppHeader.vue` desktop squircle cluster and the mobile header section.
...
- `src/components/common/AppHeader.vue` — mount `<NotificationsBell />` (desktop cluster + mobile header).
...
3. **Deep-link (`entityDeepLink.test.ts`):** every `ResultType` maps to the same path/query as the pre-extraction `selectResult` switch.
