...
> **UPDATE (2026-06-02, post-gate):** The managed provider changed after empirical testing. **Managed primary = Tinfoil-direct (`qwen3-vl-30b` at `https://inference.tinfoil.sh/v1`), NOT Phala/RedPill.** RedPill was rejected — it routes non-deterministically (~50/50 Phala/Tinfoil, pins ignored) and terminates TLS at its gateway. Tinfoil-direct gives a deterministic, named, **live-verified** attested enclave + **in-enclave TLS + EHBP** (so our proxy can be a **blind ciphertext forwarder** that never sees the document). **Gate 1 (quality) PASSED** (`qwen3-vl-30b`: 6/6 images + 5/5 PDFs); Gate 2 (DPA) now targets **Tinfoil**; a new **Gate 3** = integrate Tinfoil's verification SDK + EHBP before claiming "no intermediary sees the document." Authoritative current state: **ADR-030** + `docs/research/2026-06-02-tinfoil-phase1-gates.md`. References to "Phala/RedPill" below are retained as the original (now-rejected) plan; read the managed provider as **Tinfoil** unless the text is describing the rejected option. PDF support is **validated** (render page-1 to image) and can be in v1.
...
**Decided direction (locked with greg):** tiered AI, preference order **on-device → BYOK → verifiable-TEE managed (Tinfoil-direct) → documented Gemini Flash-Lite (Vertex) fallback**; split by modality (on-device for text now, consent-gated cloud for document images); first wedge = invitation/event → prefilled calendar activity. Gate 1 (quality) **passed**; Gate 2 (Tinfoil DPA) + Gate 3 (Tinfoil verification-SDK/EHBP integration) remain.
...
1. **Decision record (documentation deliverables):**
   ...
   2. An ADR (`docs/adr/030-private-ai-tiered-architecture.md`) recording the tiered architecture, the cloud privacy spectrum, the provider verdict (Tinfoil-direct primary, Gemini/Vertex fallback), the data-minimization + per-action-consent principles, and the "verifiable ≠ trustless" honesty constraint.
...
2. **Validation gates (must pass before any provider-committed implementation):**
   ...
   2. Confirm Tinfoil zero-retention / GDPR Article 28 processor terms / children's-data suitability. If either gate fails, switch the managed path to the Gemini Flash-Lite fallback (same abstraction).
...
4. **Tier framework (extensible foundation, not all tiers built now):**
   ...
   2. Wire the existing `Settings.aiProvider` / `aiApiKeys` into a real Settings UI panel (BYOK entry + tier selection + a friendly privacy explanation). Default tier = beanies-managed (Tinfoil) with explicit consent; BYOK optional.
...
5. **Managed-tier backend:** a thin server-side proxy (extending the existing AWS Lambda/Terraform/registry pattern) that holds the **Tinfoil** API key, enforces per-family rate limits, and retains nothing. A browser PWA cannot safely hold the provider key. **Target design: a blind ciphertext forwarder** — with Tinfoil's EHBP, the client encrypts the document body to the attested enclave and the proxy forwards ciphertext it cannot read (key custody + rate-limit only). Integrating Tinfoil's verification SDK + EHBP is Gate 3.
...
- **"Verifiable" ≠ "trustless."** The ADR and any user copy must not overclaim. Attestation proves hardware + loaded-code identity rooted in AMD/Intel/Tinfoil PKI; it does not make the silicon independently auditable. Tinfoil's in-enclave TLS + EHBP additionally enable a genuine "no plaintext intermediary" claim — but only once the verification SDK is integrated (Gate 3); until then scope to "attested confidential compute + zero retention."
- **Gates are real gates.** Gate 1 (quality) passed on Tinfoil; do not claim "no intermediary sees the document" before Gate 3 (verification-SDK/EHBP integration) is verified, and confirm the Tinfoil DPA (Gate 2) before launch. The provider abstraction means a gate failure swaps the implementation to Gemini Flash-Lite (Vertex) without touching the feature code.
...
1. Tinfoil remains operational with `qwen3-vl-30b-a3b-instruct` vision available at roughly the quoted pricing. (Re-verify at integration; model catalogs churn.)
...
3. Tinfoil can provide adequate zero-retention / Art-28 terms; if not, fall back to a provider that can (Gemini/Vertex/Anthropic ZDR) — the abstraction absorbs this.
...
5. `Settings.aiApiKeys` is the intended persistence for **BYOK** keys only. The existing model is provider-keyed (`AIProvider`/`AIApiKeys`, `models.ts:1096-1102`) and has **no tier field** — the tier model requires a deliberate model extension (see Phase 4), not a pretend-fit. The managed-tier (Tinfoil) key is never stored here; it lives server-side.
...
- Obtain/confirm Tinfoil data-handling + Art-28 terms in writing. Record verdict.
...
- [x] Validation gates run and recorded: image-quality test **passed** (`qwen3-vl-30b`, 6/6 images + 5/5 PDFs); managed engine chosen = **Tinfoil-direct**. [ ] Tinfoil DPA (Gate 2) + verification-SDK/EHBP (Gate 3) before launch.
...
1. **Validation spike:** ✅ done — ran the real-image corpus through Tinfoil `qwen3-vl-30b` (6/6 images + 5/5 PDFs clean); chosen managed = Tinfoil. Remaining: confirm Tinfoil DPA terms (Gate 2).
