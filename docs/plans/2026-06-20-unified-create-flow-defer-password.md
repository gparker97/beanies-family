...
- **`signUp` must create the session/family/owner WITHOUT a usable password — and the empty-hash window is verified safe.** Add a `deferPassword` path to the SINGLE existing `signUp`/`buildOwnerDoc` (no clone). When deferred, skip `hashPassword(params.password)` (`:534`) and pass `passwordHash: DEFERRED_PASSWORD_HASH` (a single named `''` constant co-located with `buildOwnerDoc`, so the sentinel is greppable and the fail-closed guard in `createNewFile` references the same constant). The `signUp` param becomes a **discriminated union** so the two modes can't be mixed: `{ deferPassword: true; password?: never } | { deferPassword?: false; password: string }`. Verified readers of the owner's `passwordHash` between step-1 `signUp` and the finish-phase `createNewFile`:
  ...
  - **WATCH (verified):** `buildOwnerDoc` passes `requiresPassword: false` literally (`authStore.ts:453`), but `familyMemberRepository.applyDefaults` forces `requiresPassword = !member.passwordHash`, **overriding** it. So an empty-hash owner is stored with `requiresPassword: true`. This is _latent but safe_ pre-pod: the only consumer, `familyStore.normalizeRoles`, only **elects** a new owner when `owners.length === 0`; our owner keeps `role: 'owner'`, so it is never demoted. The wart is **self-correcting**: the finish phase's `rehydrateOwnerDoc` re-runs `buildOwnerDoc` with the real hash, recomputing `requiresPassword` to `false` before the pod is written. **Acceptance test must assert the stored owner ends with `requiresPassword === false` and a non-empty `passwordHash` after the finish phase.**
...
5. **VERIFIED:** the desktop popup path returns `{ status: 'connected' }` without reload, so it can enter the finish surface via the host-owned `activeView` flip with the provider still live (`isTokenValid()` → `finishOnDrive` short-circuits to `finalizePod`). The seam is a NEW `CreatePodView` emit + a NEW `LoginPage` handler setting `activeView = 'resume-setup'` (no existing emit does this — verified).
...
- `src/components/login/ResumePodSetup.vue` — becomes the single create-finish surface: existing `identity`→`rehydrateOwnerDoc`→`finishOnDrive`→`finalizePod` PLUS a new `members` phase rendering `<CreateMembersStep>` after a successful pod write, then `SetupProgressModal` → `/nook`. `finalizePod` no longer emits `signed-in` on success — it sets `phase = 'members'`. Auto-load/open-existing/retry success paths unchanged (still emit `signed-in` directly). Add the phase-reachability table + update the recovery-vs-create doc-comment.
...
- [ ] **Desktop (popup):** same outcome via the finish-surface `activeView` flip — step 1 (no password) → Drive popup → **password once** → **add members** → `/nook`. One completion path shared with iOS, no route change / re-mount.
