...
5. `activityStore.test.ts`: `backfillReminderMinutes` issues **one** batch, never calls `updateActivity`, patches `activities.value` once, writes the marker last, and writes **no** marker when the list is empty or the member can't edit.
...
14. Back-fill: existing activities show 30 in the editor and schedule reminders; **a vacation trip produces no 09:00-per-day storm**; any linked recurring payment that was deactivated is **still deactivated**.
...
## Open Decision (needs greg before the back-fill ships)

**Google Calendar double-notification — and the back-fill makes it corpus-wide on day one.** `reminderMinutes` is folded into `computePushHash` (`activityToGoogleEvent.ts:170`), so the back-fill changes the push hash of **every** in-window pushable activity. `calendarSyncStore` watches `activityStore.activities` and reconciles on any change (`calendarSyncStore.ts:958-966`), so the single batch triggers one reconcile in which every already-synced event is dirty and re-pushed, carrying a `{ method: 'popup', minutes: 30 }` override (`activityToGoogleEvent.ts:75-78`). This is not "a surprise on new activities" — it is a fleet-wide behaviour change for every Calendar-connected family.

**The re-push cost is paid on either answer.** Suppressing the overrides does not un-dirty the hash, so the one-off burst happens regardless: N Google `patch` calls (pooled at `MAX_INFLIGHT`, `calendarSyncStore.ts:533`) plus N `lastPushedHash` link writes, on the first boot after this ships, bounded by `isPushable`'s window. It is throttled and self-limiting, and the link writes touch `calendarConnections` not `activities`, so it cannot self-loop — but say it in the changelog so the first "why did my calendar just churn" report is answerable.

- **Keep it (recommended).** The popup is genuinely correct — the event really does have a 30-minute reminder — and it is the _only_ alert for anyone on web/desktop, where beanies cannot fire an OS notification. Call it out in the changelog.
- **Suppress it.** Export `overrides: []` regardless of `reminderMinutes`, treating beanies as the sole reminder owner. Cleaner for native users; silently removes the only reminder web/desktop users would get.

Recommendation: **keep**, and say so in the changelog. Everything else in this plan is independent of the answer, but this needs deciding **before** the back-fill ships, not after.
