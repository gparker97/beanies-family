...
- **PR 1 (Phase A + B)** shipped 2026-08-13 as `0.9.9R10` (instrumentation, dead-code removal, `dirty`/`changed` gates, no-op snapshot suppression, duplicate-metadata removal). Soaked in prod ≥3 days (prod reached `0.9.10R8`).
- **PR 2 (Phase C + D) — code IMPLEMENTED 2026-08-17** across five commits (C-0 pure module → C-1 probe/ack → C-2..C-6 marker/baseline/guard machinery → tests). The revision-gated open-read guard, the persisted worker baseline row (all C4a/C10/C11/C18/C16/C19 invariants), the single `remoteChanged`/`shouldSkipOpenRead` comparator (deleting `checkForConflicts`/`getFileTimestamp`/`setLastKnownFileTimestamp`), the write-ack plumbing (one fewer Drive round-trip per save), the load-probe reorder (C13), and `runPostLoadDriveHousekeeping` (load/skip parity) all landed. Full unit suite green (**4398+**), `type-check` + `lint` + `npm run build` clean. New tests: pure `remoteBaseline` table (17), provider ack/marker (4), I/O-shell (6), worker baseline-commit termini (3), cache-row isolation (3).
- **`/code-review max` PASS DONE 2026-08-17** (14 findings). Fixed in one remediation commit: **A1** the 10s poll now surfaces a genuine Drive 404 as the missing-file banner + stops polling (`remoteChanged` classifies 404 as `file-not-found`, distinct from `auth`); **B1** `installProvider` uses `syncNow(true)` so a freshly-created/migrated file is never false-blocked with "File has newer data" (this reliably broke every Drive migration); **C1** `doSave`/`fetchAndMergeRemote` guard the baseline commit against a family switch landing mid-op; **1117** `doSave` re-probes `getRemoteMarker` on an unparseable Drive ack rather than nulling the revision basis; the daily-user **double-probe** removed (trust-window checked before probing); `remoteChanged`'s catch now firehoses the degradation; local providers classify as clean `no-baseline` not `open-fail-open`; `RemoteMarker`/`WriteAck` hoisted to `storageProvider.ts` (layering); needs-password branches route through `runPostLoadDriveHousekeeping`. 4413 tests + build green.
- **Documented limitations (accepted, NOT blocking — flagged to greg):** (a) the skip path does not re-push a local edit that was persisted to the worker cache but whose Drive save never flushed before a force-kill — bounded to ≤1h (trust expiry) or the next edit, data-safe locally, cross-device-delayed only; a follow-up could track last-Drive-pushed heads to close it. (b) `recovered`-path `clearRemoteBaseline` is best-effort (swallowed) — a transient IDB delete failure + offline read + coincidental version match could serve stale on a later open; extremely narrow. (c) the whole guard rests on Drive `version` advancing on every content write — safe by construction (a non-advancing field just fails open to today's always-read) but confirmed only by the after-capture's absence of `no-revision`.
- **REMAINING before close (greg):** (1) the **after-measurement** on a real 2-3MB `.beanpod` (before-baseline captured 2026-08-17: `rec=2 reads=1 writes=0` — see `docs/PERFORMANCE.md` §10; after PR 2 deploys, expect `open-skip rec=1 reads=0`); (2) deploy + on-device verify. NOT deployed.
