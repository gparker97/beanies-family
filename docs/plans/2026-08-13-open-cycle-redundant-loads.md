...
- **REMAINING before close (greg):** (1) the **before/after measurement** on a real 2-3MB `.beanpod` for `docs/PERFORMANCE.md` (the soak-gate before-baseline + `version`/`headRevisionId` confirmation — both still unchecked in the audit, and CloudWatch-only, so mine to record but greg's to capture); (2) on-device verify after deploy (open read-only twice → zero Drive read/write on the 2nd; a peer key-rotation forces a read; Refresh still fetches a changed file); (3) a `/code-review max` pass on the PR-2 diff is strongly advised before deploy — the recurring-edit fix's first pass introduced 8 regressions that only code-review caught, and this change is the same data-safety class. NOT deployed.
