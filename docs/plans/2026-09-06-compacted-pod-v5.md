...
6. Restore drill: restore the safety copy on A through Settings, Load another Family Data File; confirm B ADOPTS rather than reverting (watch B's console for `open terminus adopted`, not `kept-local`); confirm the restored family then compacts again cleanly and stays 5.0. Then sign in on a THIRD, cache-free browser profile and confirm it adopts without minting (no `adopted` burst on A and B afterwards).
   - ⚠️ **NAME THE BROWSER, AND RUN IT ON BOTH.** As originally written this drill could not pass on the platform a developer would naturally use. `openAndLoadFile` had two arms and neither consulted the provider: on **Chromium desktop** the File System Access arm re-homed the family to a LOCAL file, so B was structurally unable to follow and the drill reported a failure of R4 while R4 was working perfectly. On **native, Firefox and Safari** the same button left the family on Drive and the restore propagated. Fixed 2026-09-07 (`docs/plans/2026-09-07-cache-read-failure-and-provider-aware-restore.md`): the picker now follows the family's provider and a restore no longer changes the provider, `fileId` or registry pointer. Run the drill **once on Chromium desktop and once on Firefox**; both must now behave identically, both must be able to SELECT the Drive safety copy, and after both the owner's provider config must still name the original pod.
   - **Known limitation, not a drill failure:** `preserveLocalKeyDicts` unions the restoring device's current key dicts into the restored envelope, so members who joined after the copy was taken keep their wraps. It cannot cover a full family-key ROTATION performed after the copy — in that case the restored envelope carries the old `keyId`.
