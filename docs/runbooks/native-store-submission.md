...
| Diagnostic events (timings, provider/sync state, data-connection config-heal state (had-config/session/registry-fileId/token-valid booleans), incremental-sync phase/reason/counts, cache-persist kind/error, doc-worker recovery method/attempt, biometric-passkey PRF enable/eval/unwrap outcome, native-biometric enable/unlock outcome + hardware key backing, on-device reminder scheduling (how many reminders were armed, whether the cap clipped the list, how many records were skipped, the default lead, the notification + exact-alarm permission states, which scheduling stage failed, the delivered reminder's kind, a bucketed delivery-lateness band, and whether the device timezone changed), browser UA, os, online, connection type, web_storage, build_sha, perf) | Slack + telemetry firehose     | Diagnostics → Other Diagnostic Data | App info & performance → Diagnostics | App functionality (reliability)                                 | Yes¹            | No        |
