...
> **`notif_count` semantics changed 2026-07.** On the `reschedule` event it is now the count actually ARMED, not the count desired — pre-2026-07 data is not comparable. On the `notif_error_stage: 'schedule'` error event it remains the count ATTEMPTED, so any fleet aggregate over `notif_count` must filter on message / `notif_error_stage`., Google token lifecycle (surface `google-token-lifecycle`, #62: which grant — drive/calendar — the op — mint/revoke/recovery — the outcome — ok/failed/skipped — a short reason enum and the trigger; all PII-free closed enums, never a token value), browser UA, os, online, connection type, web_storage, build_sha, perf) | Slack + telemetry firehose | Diagnostics → Other Diagnostic Data | App info & performance → Diagnostics | App functionality (reliability) | Yes¹ | No |
