...
| Diagnostic events (timings, provider/sync state, save-status indicator transitions (a fixed `save_status` enum — saving/critical/degraded/saved/hidden — plus a `consecutive_failures` count), data-connection config-heal state (had-config/session/registry-fileId/token-valid booleans), incremental-sync phase/reason/counts, cache-persist kind/error, doc-worker recovery method/attempt, biometric-passkey PRF enable/eval/unwrap outcome, native-biometric enable/unlock outcome + hardware key backing, on-device reminder scheduling (how many reminders were armed, whether the cap clipped the list, how many records were skipped, the default lead, the notification + exact-alarm permission states, which scheduling stage failed, the delivered reminder's kind, a bucketed delivery-lateness band, whether the device timezone changed, the device's default activity lead, how many items were intentionally filtered out by rules, a one-off count of activities touched by the 2026-07 reminder back-fill, and Helpful Hints reconcile outcomes (how many hint to-dos were generated / expired / pruned, the running total, a per-record skip count, whether the feature + family master switch are on, the closed hint-type enum + operation, on a hint being ticked done, and per-reason skip counts for why a candidate was not generated — e.g. no date of birth, outside the lead window), and what happened when a reminder notification was tapped (navigated / deferred until the family data loaded / ignored because it carried no target — no entity id or link is ever sent)) |

> **`notif_count` semantics changed 2026-07.** On the `reschedule` event it is now the count actually ARMED, not the count desired — pre-2026-07 data is not comparable. On the `notif_error_stage: 'schedule'` error event it remains the count ATTEMPTED, so any fleet aggregate over `notif_count` must filter on message / `notif_error_stage`., Google token lifecycle (surface `google-token-lifecycle`, #62: which grant — drive/calendar — the op — mint/revoke/recovery — the outcome — ok/failed/skipped — a short reason enum and the trigger; all PII-free closed enums, never a token value), recurring-series occurrence handling (surfaces `activity-override` / `activity-scope-edit` / `activity-series-delete` / `activity-fee-sync` / `recurring-item-scope` / `calendar-sync`, 2026-08: the calendar date of the repeating session being edited and the date it resolved to — two `YYYY-MM-DD` values, never a title, member name or amount — plus the edit scope chosen (this-one / this-and-future / all), whether the session was deliberately moved, a short outcome enum or count, and a comma-joined list of MODEL FIELD NAMES that were stripped from a derived record; all PII-free), meal planner (surfaces `meal-planner` / `plan-export`, #27/#67: the meal `kind` (recipe/eat_out/leftovers/skip/other) and `slot` (breakfast/lunch/dinner/snack), whether a quick-add was used, the share `share_scope` (day/week), whether a copy `overwrote` the target week, and — for the one-page image/PDF export — the `format` (image/pdf) and the pipeline `stage` (render/rasterize/pdf/deliver) on a failure; all PII-free fixed enums/bools, never a recipe name, note, cook or guest name), unified recurrence engine (surface `recurrence`, #70: which surface a repeating rule lives on — transaction/activity/list — the cadence unit — day/week/month/year — a small interval integer, the end KIND ONLY — never / on-a-date / after-N, never the end date itself — and a short adapter-fallback reason enum; all PII-free fixed enums/ints, never a description, title, amount or date), , recipe capture (surfaces `recipe-extract` / `recipe-fetch`, #72: which rung of the capture ladder produced the recipe — `extraction_path`, a closed enum of document/jsonld/page_text/youtube_link_followed/youtube_captions — plus two small integer counts, `inferred_count` (how many lines the model filled in itself) and `ingredient_count`; reuses the existing `action`, `kind` and `error_code` keys. All PII-free enums and counts — never the source URL, the fetched page text, a video transcript, a dish name or any recipe content. **Extended 2026-09 (#86, dish images):** the same surface now also emits which rung of the IMAGE ladder supplied a recipe's photo, or why none did, via the ALREADY-DECLARED generic `detail` and `count` keys — `detail` is a closed enum naming the rung (jsonld / og_image / og_secure / twitter / twitter_src / link_rel / thumbnail / youtube_thumb / other) or the reason (no_candidates / all_failed / cloud_required / at_cap / store_rejected / unsafe_candidate), and `count` is a small integer of how many candidates were tried. **NO NEW KEYS AND NO NEW DATA CATEGORY** — never an image URL, an image host, or the bytes), share capture (surfaces `share-target-ingest` / `recipe-extract`, #64: how a share reached the app and what shape it was — `file_count` (an integer count of attachments, also used for the count of share adapters that started), `cold_start` (whether the share arrived at launch rather than at resume), and a `detail` label naming the DELIVERY MECHANISM and outcome — `link` / `file` / `open_in` / `launch_url` / `empty_file`, plus, on iOS only, the Share Extension's own run trace, since an extension is a separate process that cannot log for itself. That trace is a compact `k=v` string of counts and closed enums: the stage reached (`staged` / `nothing_staged` / `notified` / `notify_failed`), how many items were offered and how many were staged, whether notifications are permitted, whether the app opened directly, and `offered` — the UNIFORM TYPE IDENTIFIERS the sending app declared, e.g. `public.png`, `public.plain-text`, `com.adobe.pdf`. Those are fixed Apple type constants describing the KIND of item, never a file name and never its contents. Reuses the existing `action`, `os` and `error_code` keys. NEVER the shared URL, the file name, the file bytes, the video title, or any extracted content), bean identity (surfaces `member-colour` / `activity-celebration` / `activity-owner-resolve`, 2026-09: how a bean's display colour was resolved and why a card was or was not decorated as a celebration — reuses the existing `action`, `kind` and `error_code` keys plus one NEW generic key, `count`, a small integer (how many beans hold no colour, how many palette hues are taken, how many activity records reference a member who no longer exists). All PII-free integers and closed enums — never a bean's name, an activity title, or the keyword that matched), keystore reconcile (surface `native-biometric`, #82: how much device-local biometric key material was found, adopted back into the registry after a reinstall, or removed — reuses the ALREADY-DECLARED `action`, `count`, `detail`, `error_code` and `os` keys. `detail` is a fixed `k=v` string of small integers under fixed labels (`adopted=`/`registered=`/`legacy=`/`malformed=`, `targets=`/`failed=`, `deleted=`) and `count` is a small integer. **NO NEW KEYS AND NO NEW DATA CATEGORY** — never a family id, a member id, an account string, or any key material), beanie wall sizing (surface `beanie-wall`, #96: whether the wall could be shown on this screen and how densely it drew itself — FOUR NEW KEYS, `viewport_w` / `viewport_h` / `screen_w` / `screen_h`, each a coarse integer of CSS pixels, plus the already-declared `kind` naming which floor refused the device (`device-too-small` / `box-too-small`) or which density tier rendered (`full` / `mid` / `compact`). These exist because the old event could say only THAT the wall was refused, never on what size of screen, so an entire class of 8-inch tablets was turned away without anything in the logs looking wrong. Screen and viewport dimensions are device characteristics with no identifier value beyond what a user-agent string already exposes, and carry no user content — never a family name, a member, an activity title or a date), QR decoding (surface `qr-decode`, 2026-09: which attempt of the decode ladder read a scanned code, or that every attempt was exhausted — reuses the ALREADY-DECLARED `action`, `kind`, `error_code` and `detail` keys. `kind` is a closed enum naming the WINNING rung (`native` / `full-luma` / `full-blue` / `crop-blue` / `large-blue`) and is absent entirely when no rung read the code; `detail` is `origin=<entry point>;tried=<comma-joined rung list>`, where the entry point is a fixed label (`profile-menu` / `cold-entry` / `recovery-kit`) and the list is the same closed rung enum plus `native-threw` for a platform decoder that constructed and then failed. It exists because a decode that found nothing emitted NOTHING, so the most common outcome was invisible and the contrast fix behind it would have been unmeasurable. **NO NEW KEYS AND NO NEW DATA CATEGORY** — never the photo, its bytes, its dimensions, or the decoded payload), browser UA, os, online, connection type, web_storage, build_sha, perf) | Slack + telemetry firehose | Diagnostics → Other Diagnostic Data | App info & performance → Diagnostics | App functionality (reliability) | Yes¹ | No |
...
> | Member count (`memberCount`, a bare integer — how many members the family roster holds; no names, ids, or roles) | Family registry PUT | Usage Data → Product Interaction | App activity → App interactions | Analytics (total users, members-per-family) | Yes² | No |
