...
| Diagnostic events (timings, provider/sync state, incremental-sync phase/reason/counts, cache-persist kind/error, doc-worker recovery method/attempt, biometric-passkey PRF enable/eval/unwrap outcome, native-biometric enable/unlock outcome + hardware key backing, browser UA, os, online, connection type, web_storage, build_sha, perf) | Slack + telemetry firehose     | Diagnostics → Other Diagnostic Data | App info & performance → Diagnostics | App functionality (reliability)                                 | Yes¹            | No        |
