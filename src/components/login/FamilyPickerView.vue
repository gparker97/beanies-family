...
<template>
  <div
    class="dark:bg-surface-raised mx-auto max-w-[540px] rounded-3xl bg-white p-5 shadow-xl sm:p-8"
  >
    <!-- Back button -->
    <button
    ...
    <div class="mb-6 text-center">
      ...
      />
      <h2 class="font-outfit dark:text-ink text-lg font-bold text-gray-900">
    ...
    <template v-else>
      ...
      <div v-if="families.length === 0" class="py-8 text-center">
        <div
          class="dark:bg-surface-overlay mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-2xl bg-gray-100"
        >
          <svg
            class="dark:text-ink-faint h-7 w-7 text-gray-400"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
      ...
      <div v-else class="space-y-3">
        <div v-for="family in families" :key="family.id" class="flex items-center gap-2">
          <button
            class="hover:border-primary-500/40 dark:hover:border-primary-500/30 dark:border-line-strong flex min-w-0 flex-1 items-center gap-3 rounded-2xl border-2 border-gray-200 p-3 text-left transition-all hover:-translate-y-0.5 hover:shadow-lg sm:p-4"
          ...
          >
            ...
            <div class="min-w-0 flex-1">
              <span class="font-outfit dark:text-ink block font-semibold text-gray-900">
                {{ family.name }}
              </span>
              <!-- Provider pill -->
              <CloudProviderBadge
          ...
          <button
            ...
            class="dark:hover:text-danger-lift shrink-0 rounded-xl p-2.5 text-gray-400 transition-colors hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-900/20"
        ...
        <button
          class="dark:text-ink-soft dark:hover:text-ink mt-4 w-full text-center text-sm text-gray-500 hover:text-gray-700"
          @click="$emit('load-different-file')"
        >
