...
export function useHelpfulHints(): void {
  ...
  async function reconcile(): Promise<void> {
    ...
    let generated = 0;
    // Hint to-dos are generated by the app on a boot/watch tick, not by a user.
    // `feature_used` is an interactive event, so reporting them would both
    // un-bounce a visitor who did nothing and inflate `todo` adoption with rows
    // no human created.
