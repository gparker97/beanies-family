...
const UTILITY_ICONS = {
  ...
  link: {
    paths: [
      'M13.828 10.172a4 4 0 010 5.656l-3 3a4 4 0 01-5.656-5.656l1.5-1.5',
      'M10.172 13.828a4 4 0 010-5.656l3-3a4 4 0 015.656 5.656l-1.5 1.5',
    ],
  },
  camera: {
    paths: [
      'M3 9a2 2 0 012-2h2l2-3h6l2 3h2a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z',
  ...
  image: {
    paths: [
      ...
      'M9 9a1.5 1.5 0 100-3 1.5 1.5 0 000 3z',
    ],
  },
  'image-broken': {
    paths: [
...
const STATUS_ICONS = {
  ...
  /**
   * ⚠️ ADDED 2026-09-23 BECAUSE IT WAS MISSING, and its absence was visible in the product.
   * `ConfirmModal` has always asked for `info` on its non-danger variant, but no such icon was
   * ever registered — so `getIconDef('info')` returned undefined and every confirm and alert that
   * is not a deletion rendered `BeanieIcon`'s unknown-name PLACEHOLDER: a circle with three dots.
   * Roughly two dozen modals across the app, all looking like a broken image. The placeholder is
   * deliberately quiet by design (see `BeanieIcon`), which is why nothing ever flagged it.
   */
  info: {
    paths: ['M12 16v-4M12 8h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z'],
  },
  'exclamation-circle': {
