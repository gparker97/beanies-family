...
describe('docOps — buildLocalOnlyCarryOps (ADR-036 stage 6)', () => {
  /** A doc with `entries` written into `collection`. */
  const withColl = (collection: string, entries: Record<string, unknown>): Doc =>
    Automerge.change(base(), (d) => {
      const target = (d as unknown as Record<string, Record<string, unknown>>)[collection]!;
      for (const [id, entity] of Object.entries(entries)) target[id] = entity;
    });

  /** The ops as a flat array, whatever shape the composer chose. */
  const opsOf = (r: { op: unknown }): Array<Record<string, unknown>> => {
    const op = r.op as Record<string, unknown> | null;
    if (!op) return [];
    return op.op === 'batch' ? (op.ops as Array<Record<string, unknown>>) : [op];
  };

  it('emits one `set` per entity present locally and absent from the target', () => {
    const local = withColl('todos', { a: txn('a'), b: txn('b') });
    const target = base();
    const res = buildLocalOnlyCarryOps(local, target);
    ...
    expect(opsOf(res).every((o) => o.op === 'set' && o.collection === 'todos')).toBe(true);
  });

  it('never touches an entity present in BOTH — the compacted copy stands', () => {
    // The additive-only invariant. If this ever emitted, the carry could overwrite
    // a value the family has already saved, with no common ancestor to justify it.
    const local = withColl('todos', { shared: txn('shared', { title: 'local' }) });
    const target = withColl('todos', { shared: txn('shared', { title: 'remote' }) });
    expect(buildLocalOnlyCarryOps(local, target)).toEqual({ op: null, count: 0 });
  });

  it('never deletes an entity present only in the TARGET', () => {
    const local = base();
    const target = withColl('todos', { theirs: txn('theirs') });
    const res = buildLocalOnlyCarryOps(local, target);
    expect(res).toEqual({ op: null, count: 0 });
    expect(opsOf(res).some((o) => o.op === 'delete')).toBe(false);
  });

  it('answers `{ op: null, count: 0 }` when there is nothing to carry', () => {
    expect(buildLocalOnlyCarryOps(base(), base())).toEqual({ op: null, count: 0 });
  });

  it('returns a bare op for one entity, not a batch of one', () => {
    const res = buildLocalOnlyCarryOps(withColl('todos', { only: txn('only') }), base());
    expect((res.op as Record<string, unknown>).op).toBe('set');
    expect(res.count).toBe(1);
  });

  it('carries a reference the target cannot resolve, rather than dropping the entity', () => {
    // A todo whose owner is absent from the compacted roster. `familyMembers` is
    // NEVER carried, so this dangling reference is the expected steady state — it
    // is the same one an ordinary member deletion already produces, and the
    // projection renders it as unassigned. Dropping the todo instead would lose
    // real user work to protect against a state the app already handles.
    const local = withColl('todos', { t: txn('t', { memberId: 'gone' }) });
    const res = buildLocalOnlyCarryOps(local, base());
    expect(res.count).toBe(1);
    expect((opsOf(res)[0]!.entity as Record<string, unknown>).memberId).toBe('gone');
  });

  describe('the exclusion map is a SECURITY boundary', () => {
    // One case per excluded collection. These are the assertions that stop a
    // credential, a token, an account email or a resurrected roster member from
    // riding a data-preservation feature into the family's shared document.
    const EXCLUDED = [
      'driveConnections',
      'calendarConnections',
      'familyMembers',
    ...
    for (const collection of EXCLUDED) {
      it(`never carries a local-only ${collection} entry`, () => {
        const local = withColl(collection, { x: { id: 'x', updatedAt: '2026-01-01' } });
        expect(buildLocalOnlyCarryOps(local, base())).toEqual({ op: null, count: 0 });
      });
    }

    it('leaks no refresh token and no account email, even as a substring', () => {
      // The strongest form of the driveConnections case: not merely "no op for
      // that collection", but that the secret appears NOWHERE in what is emitted.
      const local = Automerge.change(base(), (d) => {
        const doc = d as unknown as Record<string, Record<string, unknown>>;
        doc.driveConnections!['someone@example.com'] = {
        ...
        // A carried entity alongside it, so the composer is genuinely emitting.
        doc.todos!.keep = txn('keep');
      });
      const res = buildLocalOnlyCarryOps(local, base());
      expect(res.count).toBe(1);
  ...
  it('never emits the `settings` or `podLineage` singletons, even when they differ', () => {
    // Structural, not incidental: `MutationOp.collection` is `CollectionName`,
    // which excludes the singletons, and the loop iterates COLLECTION_NAMES. An op
    // stamping the OLD lineage onto the NEW document would be self-inflicted
    // lineage corruption with no external cause.
    const local = Automerge.change(base(), (d) => {
      (d as unknown as Record<string, unknown>).settings = { baseCurrency: 'SGD' };
      (d as unknown as Record<string, unknown>).podLineage = { id: 'OLD', seq: 1 };
    });
    const target = Automerge.change(base(), (d) => {
      (d as unknown as Record<string, unknown>).podLineage = { id: 'NEW', seq: 2 };
    });
    const res = buildLocalOnlyCarryOps(local, target);
    const touched = opsOf(res).map((o) => o.collection);
    expect(touched).not.toContain('settings');
    expect(touched).not.toContain('podLineage');
  });
});
