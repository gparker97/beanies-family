...
describe('driveRequest — a 401 drops the cached access token', () => {
  ...
  it('clears UNCONDITIONALLY, because a revocation kills the whole grant', async () => {
    // ⚠️ A COMPARE-AND-CLEAR VARIANT WAS TRIED AND REVERTED. Only clearing when
    // the cached token still equals the failed one looks safer under concurrency
    // — but Google's revoke kills every token of the grant at once, so a late
    // 401 from a superseded token means the replacement is dead too. Skipping
    // the clear there leaves `isTokenValid()` blessing a dead grant for up to an
    // hour, which is the exact loop this hook exists to break. A redundant
    // refresh is the cheaper mistake.
    global.fetch = mockFetch({ error: { message: 'Invalid Credentials' } }, 401);
    await expect(readFile('some-other-token', 'file-1')).rejects.toThrow(DriveApiError);
    expect(mockInvalidateAccessToken).toHaveBeenCalledTimes(1);
  });
